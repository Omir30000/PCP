...
import DadosPoco from './DadosPoco';
  ...
  const getDefaultPermissions = (role: string): Set<string> => {
    ...
    if (role === 'admin') {
      ['kanban', 'vendas', 'dashboard_vendas', 'calendario_vendas', 'produtos', 'usuarios',
       ...
       'relatorio_consumo_agua', 'relatorio_consumo_agua_sku', 'dados_poco',
    ...
    <div className={`min-h-screen flex overflow-hidden font-sans selection:bg-[#facc15]/30 bg-[#0a0a0a] text-slate-100`}>
          ...
          {(screenPermissions.has('relatorio_boletim_pro') || screenPermissions.has('relatorio_boletim_ai') || screenPermissions.has('downtime_horas_ia') || screenPermissions.has('relatorio_consumo_agua') || screenPermissions.has('relatorio_consumo_agua_sku')) && (
            <>
              ...
              <div className={`space-y-0.5 transition-all duration-500 overflow-hidden ${isAIReportsOpen || !isSidebarExpanded ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'}`}>
                ...
                {screenPermissions.has('relatorio_consumo_agua_sku') && <NavItem id="relatorio_consumo_agua_sku" icon={Boxes} label="Boletim por SKU" isSubItem={isSidebarExpanded} />}
                {screenPermissions.has('dados_poco') && <NavItem id="dados_poco" icon={Droplet} label="Dados do Poço" isSubItem={isSidebarExpanded} />}
      ...
      <main className={`flex-1 min-w-0 relative overflow-y-auto h-screen scroll-smooth transition-colors duration-500 bg-[#0a0a0a]`}>
        <div className="relative z-10 px-4 py-6 md:px-6 md:py-8 lg:px-10 lg:py-10 max-w-[1800px] mx-auto pb-24 lg:pb-10 animate-in fade-in duration-500">
          ...
          {activeTab === 'dados_poco' && <DadosPoco />}
            ...
            <div className="space-y-1 overflow-y-auto no-scrollbar flex-1">
              ...
              {(screenPermissions.has('relatorios') || screenPermissions.has('analise_disponibilidade')) && (
                <>
                  ...
                  {screenPermissions.has('relatorio_consumo_agua_sku') && <NavItem id="relatorio_consumo_agua_sku" icon={Boxes} label="Boletim por SKU" onClick={() => setIsMobileMenuOpen(false)} />}
                  {screenPermissions.has('dados_poco') && <NavItem id="dados_poco" icon={Droplet} label="Dados do Poço" onClick={() => setIsMobileMenuOpen(false)} />}
