...
const TURNOS = [
  { id: '1', nome: 'T1', fullName: 'Turno 1 - Manhã', color: 'text-[#facc15]', border: 'border-[#facc15]/30', labelBg: 'bg-[#facc15]' },
  { id: '2', nome: 'T2', fullName: 'Turno 2 - Noite', color: 'text-blue-400', border: 'border-blue-400/30', labelBg: 'bg-blue-400' }
];
...
const ProgramacaoKanban: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col bg-[#050505] text-slate-300 font-sans overflow-hidden p-2">
      ...
      {/* Toast Feedback */}
      {toast && (
        <div className={`fixed top-10 right-10 z-[300] px-8 py-4 rounded-[20px] flex items-center gap-4 animate-in slide-in-from-right shadow-[0_30px_60px_rgba(0,0,0,0.6)] border ${toast.type === 'success' ? 'bg-[#22c55e] border-white/20 text-black' : 'bg-rose-600 border-white/20 text-white'
          }`}>
      ...
      <div className="flex items-center justify-between mb-4 px-4 h-16 shrink-0">
        <div className="flex items-center gap-4 bg-slate-900/50 p-1.5 rounded-2xl border border-white/5 shadow-lg">
          <button
        ...
        <div className="flex items-center gap-6">
          <button
            onClick={exportarPDF}
            className="flex items-center gap-3 bg-slate-900/40 hover:bg-slate-800 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/5 transition-all shadow-xl active:scale-95 group"
          >
            <Printer className="w-4 h-4 group-hover:scale-110 transition-transform text-[#facc15]" /> EXPORTAR PDF
          </button>

          <div className="flex items-center gap-2 px-6 py-3 bg-slate-900/40 border border-[#facc15]/20 rounded-full shadow-[0_0_15px_rgba(250,204,21,0.1)]">
            <div className="w-1.5 h-1.5 rounded-full bg-[#facc15] animate-pulse" />
            <span className="text-[9px] font-black text-[#facc15] uppercase tracking-widest">Sincronizado via Nexus Core</span>
          </div>
        </div>
      </div>
          ...
          <div className="p-4 border-b border-white/5 bg-black/20">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/5 rounded-xl py-2.5 pl-10 pr-4 text-[9px] font-black uppercase tracking-widest outline-none focus:border-[#facc15]/30 transition-all text-white placeholder:text-slate-700"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-2.5">
            {produtos.filter(p => p.nome.toLowerCase().includes(skuSearchTerm.toLowerCase())).map(prod => (
              <div
                ...
                className="bg-black/40 border border-white/5 p-4 rounded-xl hover:border-[#facc15]/30 transition-all cursor-grab active:scale-95 group relative shadow-lg"
              >
                ...
                <div className="flex items-center gap-1.5 text-[8px] font-bold text-slate-600 uppercase">
                  <Zap className="w-3 h-3 text-[#facc15]" /> CAP: {prod.capacidade_nominal?.toLocaleString()} UN
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-2 h-full min-w-[1200px]">
            {getDiasSemana.map(dia => (
              ...
              >
                ...
                <div className="bg-slate-900/60 p-3 rounded-2xl border border-white/5 text-center shadow-[0_4px_10px_rgba(0,0,0,0.5)] group-hover/col:border-[#facc15]/30 transition-all">
                  <h3 className="text-[10px] font-black text-[#facc15] uppercase tracking-[0.2em] leading-none mb-1">
                    {dia.label.split(' ')[0]}
                  </h3>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-[7px] font-black text-slate-500 uppercase relative z-10">
                          <div className="flex items-center gap-1">
                            <Box className="w-2.5 h-2.5 text-[#facc15]" />
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          ...
          <div className="bg-slate-900 rounded-2xl shadow-[0_50px_100px_rgba(0,0,0,0.5)] w-full max-w-lg relative z-10 animate-in zoom-in-95 duration-500 overflow-hidden border border-white/10 flex flex-col p-10">
            <header className="flex items-center justify-between mb-8">
              ...
              <button onClick={() => setIsDropModalOpen(false)} className="text-slate-500 hover:text-white transition-colors">
                <X className="w-8 h-8" />
              </button>
            </header>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {linhas.map(lin => (
                    <button
                      ...
                      className={`p-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedLinha === lin.id
                        ? 'bg-[#facc15] text-black border-transparent shadow-xl shadow-[#facc15]/10'
                ...
                <div className="grid grid-cols-2 gap-3">
                  {TURNOS.map(shift => (
                    <button
                      ...
                      className={`p-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedTurno === shift.id
            ...
            <footer className="mt-12 flex gap-4">
              <button
                ...
                className="flex-1 py-5 rounded-xl border border-white/5 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-all"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-[2] py-5 bg-[#facc15] text-black rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3"
              >
