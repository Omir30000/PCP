...
const RelatorioBoletim: React.FC = () => {
  ...
  const analytics = useMemo(() => {
    ...
    // Filtrar registros por turno se não for GLOBAL
    ...
    const observations = registrosFiltradosTurno
      ...
      <div ref={reportRef} className="bg-white p-0 space-y-8 print:p-0">
          ...
          <div className="flex gap-4">
            ...
            <div className="border border-slate-900 rounded-lg px-6 py-2 text-right bg-slate-900 text-white">
              ...
              <p className="text-sm font-black uppercase leading-none mt-1">
                ...
                {filtroTurno !== 'GLOBAL' && <span className="ml-2 text-[10px] text-slate-400">({filtroTurno})</span>}
              </p>
            </div>
          </div>
        </header>
              ...
              >
                ...
                <div className="flex justify-between items-end">
                  ...
                  <div className="text-right">
                    <p className="text-[8px] font-black text-slate-400 uppercase mb-0.5">Downtime</p>
        ...
        <div className="grid grid-cols-2 gap-6 items-start">
          <section className="space-y-4 break-inside-avoid">
            <h3 className="text-[10px] font-black text-slate-800 uppercase tracking-[0.3em] flex items-center gap-2">
              <AlertTriangle className="w-3.5 h-3.5 text-red-600" /> II. Matriz de Gargalos Operacionais
            </h3>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {analytics.bottlenecks.length > 0 ? analytics.bottlenecks.map((item, idx) => (
                    ...
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-300 uppercase text-[9px] font-bold italic">Sem inatividade reportada no período</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                <div key={idx} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[8px] font-black text-slate-500 uppercase tracking-wider">{formatarDataBR(obs.data)} - {obs.turno}</span>
                    </div>
